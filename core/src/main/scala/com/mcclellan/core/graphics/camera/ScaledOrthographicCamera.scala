...
case class Bounds(val bottomLeft : Vector2[Float], val topRight : Vector2[Float])
...
class ScaledOrthographicCamera(val scale : Float, val viewWidth : Int, val viewHeight : Int) {
	...
	def position : Vector2[Float] = new Vector2(cam.position.x, cam.position.y)
	def position_=(pos : Vector2[Float]) = {
